...
Steak Pie is a curated layer for the [Pi coding agent](https://pi.dev) — not a
fork. Stock Pi stays stock; Steak Pie adds only what earns its place, and
nothing else:
...
Steak Pie closes the loop after every edit. Add a verify command to your
project:

```json
// .steak-pie/config.json
{ "verify": { "command": "npm run -s typecheck", "failLimit": 2 } }
```

After each edit or write, Steak Pie runs it. Failures are appended to the
...
Projects remember through `AGENTS.md` conventions — Steak Pie ships a
...
These ship **outside** Steak Pie by design — install only what you want:
...
Steak Pie never enables companions implicitly: each one changes behavior,
so each one is an explicit choice.

## Install

```sh
pi install npm:steak-pie
```
...
orchestration and opinions — at five times the token cost. Steak Pie keeps
