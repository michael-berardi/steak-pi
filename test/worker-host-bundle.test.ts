...
describe("worker SDK host-bundle detection", () => {
  ...
  it("rejects unsupported versions, missing SDK entries, embedded hosts and missing paths", () => {
    expect(hostPiBundle(fakePi("0.86.0").bin)).toBeUndefined();
    expect(hostPiBundle(fakePi("0.88.0").bin)).toBeUndefined();
    expect(hostPiBundle(fakePi("0.87.1", false).bin)).toBeUndefined();
    const { root } = fakePi("0.87.1");
  ...
  it("covers the pinned Pi toolchain", () => {
    expect(SUPPORTED_HOST_PI_VERSIONS).toContain(pkg.devDependencies["@earendil-works/pi-coding-agent"]);
  });
});
